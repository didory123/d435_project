...
void Activity::beginActivity(const cv::Mat& initialColorMat, const cv::Mat& initialDepthMat, const cv::Mat& depthColorMapper, const cv::Rect2d& personBbox)
{
	...
	std::thread initialSnapshotThread(&Activity::initialSnapshot, this, initialColorMat, initialDepthMat, depthColorMapper, true);
	...
	cv::Mat finalDepthColor = cv::Mat(cv::Size(initialColorMat.cols, initialColorMat.rows), CV_8UC3, (void*)depth.get_data(), cv::Mat::AUTO_STEP);

	// Get final snapshot of room
	std::thread finalSnapshotThread(&Activity::finalSnapshot, this, finalRgb, finalDepth, finalDepthColor, true);
...
void Activity::initialSnapshot(cv::Mat initialColorMat, cv::Mat initialDepthMat, cv::Mat depthColorMapper, bool exportCloud)
{
	...
	if (exportCloud)
	{
		...
		plyWriter.writeASCII("./" + activityName_ + "/initialPositions.ply", outputCloud);
	}
	...
	cv::imwrite("./" + activityName_ + "/initialPositions.png", initialPositions);
...
void Activity::start()
{
	...
	while (cv::waitKey(1) < 0)
	{
		...
		{
			// Every 40 frames, if the person is found in the room store the person's movement location as pointcloud
			...
			cv::Mat depthColor = cv::Mat(cv::Size(depthMat.cols, depthMat.rows), CV_8UC3, (void*)depthFrame.get_data(), cv::Mat::AUTO_STEP);

			// Spawn a new worker thread to export the data
			//exportPointCloud(depthMat, depthColor, bbox);
			...
			workerThreads_.push_back(new std::thread(&Activity::exportPointCloud, this, depthMat.clone(), depthColor.clone(), bbox));
		}
...
void Activity::exportPointCloud(cv::Mat depthData, cv::Mat depthColorMapper, cv::Rect2d bbox)
{
	...
	*finalObjectsCloud += *helper::depthMatToColorPCL(depthData, depthColorMapper, (pipe_->get_active_profile().get_stream(RS2_STREAM_DEPTH)).as<rs2::video_stream_profile>(), bboxToFitInMat);
	...
	plyWriter.writeASCII("./" + activityName_ + "/timestamp_" + std::to_string(timeLocal.time_of_day().hours()) + "h" +
		std::to_string(timeLocal.time_of_day().minutes()) + "m" +
		std::to_string(timeLocal.time_of_day().seconds()) + "s" + ".ply", outputCloud);
}

void Activity::finalSnapshot(cv::Mat finalSnapshotColor, cv::Mat finalSnapshotDepth, cv::Mat depthColorMapper, bool exportCloud)
{
	...
	if (exportCloud)
	{
		...
		pcl::PLYWriter plyWriter;
		plyWriter.writeASCII("./" + activityName_ + "/finalPositions.ply", outputCloud);
	...
	// Store the initial snapshot as an image file
	cv::imwrite("./" + activityName_ + "/finalPositions.png", finalPositions);
