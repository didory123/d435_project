...
Marker::Marker(cv::aruco::PREDEFINED_DICTIONARY_NAME dictionary)
{
...
cv::Mat Marker::generateMarker(int markerID, int markerSize)
{
	cv::Mat markerImage;
...
cv::Mat Marker::detectMarker(cv::Mat image)
{
...
cv::Mat Marker::detectMarker(cv::Mat image, cv::Mat cameraMatrix, cv::Mat distortionCoeff)
{
	std::vector<int> ids; // Vector containing ids of markers detected from the image
	std::vector<std::vector<cv::Point2f>> corners; // Vector containing the corner coordinates of markers detected from the image
